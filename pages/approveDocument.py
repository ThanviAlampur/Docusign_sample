...
class Approve_Envelope:
    ...
    def decline_envelope(self, decline_reason):
        # Click on "Other Actions" button
        WebDriverWait(self.driver, 20).until(
            EC.element_to_be_clickable((By.XPATH, self.other_actions))
        ).click()

        # Assert visibility of required buttons
        assert all(
            WebDriverWait(self.driver, 30).until(
                EC.visibility_of_element_located((By.XPATH, xpath))
            ).is_displayed() for xpath in [
                self.finish_later_button,
                self.decline_to_sign_button
            ]
        )

        # Click "Decline to Sign" button
        WebDriverWait(self.driver, 20).until(
            EC.element_to_be_clickable((By.XPATH, self.decline_to_sign_button))
        ).click()

        # Click "Continue" button to proceed with decline
        WebDriverWait(self.driver, 20).until(
            EC.element_to_be_clickable((By.XPATH, self.decline_continue_button))
        ).click()

        # Enter decline reason
        WebDriverWait(self.driver, 20).until(
            EC.visibility_of_element_located((By.XPATH, self.decline_reason_text_box))
        ).send_keys(decline_reason)

        # Confirm the decline action
        WebDriverWait(self.driver, 20).until(
            EC.element_to_be_clickable((By.XPATH, self.dialog_decline_to_sign))
        ).click()
